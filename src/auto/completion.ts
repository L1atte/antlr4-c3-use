...
export function getScope(context: ParseTree, symbolTable: SymbolTable): any {
	if (!context) {
		return undefined;
	}
...
export function getSuggestionsForParseTree(parser: KotlinParser, parseTree: ParseTree, symbolTableFn: () => SymbolTable, position: TokenPosition) {
	...
	if (candidates.rules.has(KotlinParser.RULE_variableRead) || candidates.rules.has(KotlinParser.RULE_suggestArgument)) {
...
export function getSuggestions(code: string, caretPosition: CaretPosition, computeTokenPosition: ComputeTokenPositionFunction) {
